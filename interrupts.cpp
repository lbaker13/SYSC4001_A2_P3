...
std::tuple<std::string, std::string, int> simulate_trace(std::vector<std::string> trace_file, int time, std::vector<std::string> vectors, std::vector<int> delays, std::vector<external_file> external_files, PCB current, std::vector<PCB> wait_queue) {
    ...
    for(size_t i = 0; i < trace_file.size(); i++) {
        ...
        if(activity == "CPU") { //As per Assignment 1
            ...
            auto [child_execution, child_system_status, child_end_time] = simulate_trace(child_trace,current_time,vectors,delays,external_files,child,child_wait_queue);
            ...
            int programSize = (int)get_size(program_name, external_files);
            execution += std::to_string(current_time) + ", " + std::to_string(duration_intr)+ ", Program is " + std::to_string(programSize) + " Mb large\n";
            current_time += duration_intr;
            execution += std::to_string(current_time) + ", " + std::to_string(programSize * 15)+ ", loding program into memory\n";
            current_time += (programSize * 15);
	        if (current.partition_number >= 1 && current.partition_number <= 6) {
                free_memory(&current);
            }
            current.program_name = program_name;
            current.size = (unsigned)programSize;
	    
            bool allocation = allocate_memory(&current);

            execution += std::to_string(current_time) + ", 3, marking partition as ocupied\n";
            current_time += 3;
            ...
            auto [program_execution, program_system_status, program_end_time] = simulate_trace(exec_traces,current_time,vectors,delays,external_files,current,wait_queue);
            execution += program_execution;
            system_status += program_system_status;
